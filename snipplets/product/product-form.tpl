<div class="pt-md-3">
	...
	{% endif %}
	<div class="js-price-container price-container mb-6 p-4 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm" data-store="product-price-{{ product.id }}">
		<div id="compare_price_display" class="js-compare-price-display text-slate-400 line-through text-lg font-medium mb-1" {% if not product.compare_at_price or not product.display_price %}style="display:none;"{% else %} style="display:block;"{% endif %}>
			{% if product.compare_at_price and product.display_price %}
				{{ product.compare_at_price | money }}
			{% endif %}
		</div>
		<div class="flex items-center gap-3 mb-4">
			<span class="js-price-display text-4xl font-black text-slate-900 dark:text-white tracking-tighter" id="price_display" {% if not product.display_price %}style="display:none;"{% endif %} data-product-price="{{ product.price }}">
				{% if product.display_price %}
					{{ product.price | money }}
				{% endif %}
			</span>
			{% if product.compare_at_price %}
				<span class="bg-primary text-white text-xs font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg shadow-primary/20">
		...
		{% if product.compare_at_price > product.price or product.maxPaymentDiscount.value > 0 %}
			<div class="p-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-100 dark:border-green-800">
				<div class="flex items-center gap-2 text-green-700 dark:text-green-400 font-bold text-sm mb-1 uppercase tracking-tighter">
					<span class="material-symbols-outlined text-lg">local_offer</span>
					{{ 'Tu oportunidad de ahorro' | translate }}
				</div>
				<div class="text-xs text-green-600 dark:text-green-500/80 leading-relaxed font-medium">
					...
					{% if price_diff > 0 %}
						<div class="flex justify-between items-center py-1 border-b border-green-200/50 dark:border-green-800/50">
							<span>{{ 'Descuento directo' | translate }}</span>
							<span class="font-black">{{ price_diff | money }}</span>
						</div>
					{% endif %}
					{% if payment_diff > 0 %}
						<div class="flex justify-between items-center py-1 border-b border-green-200/50 dark:border-green-800/50">
							<span>{{ 'Ahorro por medio de pago' | translate }} (+{{ product.maxPaymentDiscount.value }}% OFF)</span>
							<span class="font-black">{{ payment_diff | money }}</span>
						</div>
					{% endif %}
					<div class="flex justify-between items-center pt-2 text-sm text-green-800 dark:text-green-300">
						<span class="font-black uppercase tracking-widest">{{ 'Ahorro Total' | translate }}</span>
						<span class="text-lg font-black">{{ (price_diff + payment_diff) | money }}</span>
					</div>
				</div>
			</div>
		{% endif %}

		{% set show_compare_price_saved_amount = false %}
